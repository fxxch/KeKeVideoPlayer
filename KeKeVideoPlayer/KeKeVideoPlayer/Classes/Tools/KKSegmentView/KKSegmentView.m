...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        self.backgroundColor = [UIColor kk_colorWithHexString:@"#EEEEEE"];
...
- (void)initUI{
    ...
    [self.backgroundImageView kk_clearBackgroundColor];
    [self addSubview:self.backgroundImageView];
    
    self.mainScrollView = [[UIScrollView alloc] initWithFrame:self.bounds];
    [self.mainScrollView kk_clearBackgroundColor];
    ...
    [self.leftMaskView kk_setBackgroundColorFromColor:[UIColor colorWithWhite:1.0 alpha:1.0] toColor:[UIColor colorWithWhite:1.0 alpha:0] direction:UIViewGradientColorDirection_LeftRight];
    ...
    [self.rightMaskView kk_setBackgroundColorFromColor:[UIColor colorWithWhite:1.0 alpha:1.0] toColor:[UIColor colorWithWhite:1.0 alpha:0] direction:UIViewGradientColorDirection_RightLeft];
...
- (void)selectedIndex:(NSInteger)aIndex needRespondsDelegate:(BOOL)aNeedRespondsDelegate{
    ...
    KKButton *button = (KKButton*)[self.buttonsArray kk_objectAtIndex_Safe:aIndex];
    if (button) {
...
- (void)deselected_needRespondsDelegate:(BOOL)aNeedRespondsDelegate{
    ...
    KKButton *oldButton = (KKButton*)[self.buttonsArray kk_objectAtIndex_Safe:_selectedIndex];
...
- (KKButton*)buttonAtIndex:(NSInteger)aIndex{
    return [self.buttonsArray kk_objectAtIndex_Safe:aIndex];
}
...
- (void)buttonEvent:(KKButton*)aButton needRespondsDelegate:(BOOL)aNeedRespondsDelegate{
    ...
    KKButton *oldButton = [self.buttonsArray kk_objectAtIndex_Safe:oldIndex];
    if (oldButton) {
...
//    CGRect windowFrame = [newButton convertRect:newButton.bounds toView:[UIWindow currentKeyWindow]];
    ...
    if (newIndex>oldIndex) {
        KKButton *readyButton = [self.buttonsArray kk_objectAtIndex_Safe:newIndex+1];
        offset = readyButton.kk_width/2.0;
    }
    else{
        KKButton *readyButton = [self.buttonsArray kk_objectAtIndex_Safe:newIndex-1];
        offset = -readyButton.kk_width/2.0;
    ...
    if (newButton.kk_right+offset>=maxX) {
        KKButton *nextButton = [self.buttonsArray kk_objectAtIndex_Safe:newIndex+1];
        if (nextButton) {
            x = MAX((nextButton.kk_right-nextButton.kk_width/2.0)-self.mainScrollView.kk_width, 0);
    ...
    else if (newButton.kk_left+offset<=minX){
        KKButton *lastButton = [self.buttonsArray kk_objectAtIndex_Safe:newIndex-1];
        if (lastButton) {
            CGFloat lastX = lastButton.kk_right-lastButton.kk_width/2.0;
            x = MAX(lastX, 0);
            [self.mainScrollView setContentOffset:CGPointMake(x, 0) animated:YES];
        }
        else{
