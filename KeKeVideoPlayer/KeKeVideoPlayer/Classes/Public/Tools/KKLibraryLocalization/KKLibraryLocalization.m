...
- (instancetype)init{
    ...
    if (self) {
        ...
        NSString *filePath = [[NSBundle mainBundle] pathForResource:@"KKLibraryLocalization.plist" ofType:nil];
...
- (NSString *_Nonnull)localStringForKey:(NSString *_Nullable)key {
    ...
    if (localText==nil || [localText isEqualToString:key]) {
        ...
        NSString *returnString = [self.keyListDictionary kk_validStringForKey:key];
        if ([NSString kk_isStringNotEmpty:returnString]) {
            return returnString;
        } else {
            return @"";
        }
