...
@implementation KKVideoPlayerSlider
...
@implementation KKVideoPlayerBarView
...
//        self.backgroundImageView.backgroundColor = [UIColor colorWithWhite:0 alpha:0.6];
        ...
        //滑动条
        self.mySlider = [[KKVideoPlayerSlider alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, 5)];
...
//        self.mySlider.maximumTrackTintColor = [UIColor colorWithRed:0.82f green:0.82f blue:0.82f alpha:1.00f];
        self.mySlider.minimumTrackTintColor = [UIColor kk_colorWithHexString:@"#BEBFBE"];
        self.mySlider.maximumTrackTintColor = [UIColor kk_colorWithHexString:@"#606160"];
...
- (void)reloadSubviewsForUIInterfaceOrientation:(UIInterfaceOrientation)currentInterfaceOrientation{
    switch (currentInterfaceOrientation) {
        ...
        case UIInterfaceOrientationPortraitUpsideDown:{
            ...
            if ([NSString kk_isStringEmpty:time1]) {
                time1 = KKLocalization(@"--:--:--");
            }
            CGSize size1 = [time1 kk_sizeWithFont:self.currentTimeLabel.font maxWidth:CGFLOAT_MAX];
            NSString *time2 = self.durationtimeLabel.text;
            if ([NSString kk_isStringEmpty:time2]) {
                time2 = KKLocalization(@"--:--:--");
            }
            CGSize size2 = [time2 kk_sizeWithFont:self.durationtimeLabel.font maxWidth:CGFLOAT_MAX];

            //播放按钮
            self.stopPlayButton.frame = CGRectMake(offsetX, 0, 60, 60);
            offsetX = offsetX + self.stopPlayButton.kk_width;
            ...
            //进度条
            offsetX = offsetX + 10;
            self.mySlider.frame = CGRectMake(offsetX, 20, (self.kk_width-offsetX-10-size2.width-15), 20);
            offsetX = offsetX + self.mySlider.kk_width;
            ...
            //更多
            self.moreButton.frame = CGRectMake(self.kk_width-60, 60, 60, 60);
        ...
        case UIInterfaceOrientationLandscapeRight:{
            ...
            offsetX = offsetX + self.backButton.kk_width;
            ...
            //进度条
            offsetX = offsetX + 10;
            self.mySlider.frame = CGRectMake(offsetX, 20, (self.kk_width-offsetX-10-size2.width-60-15), 20);
            offsetX = offsetX + self.mySlider.kk_width;
            ...
            //更多
            self.moreButton.frame = CGRectMake(self.kk_width-60, 0, 60, 60);
            break;
        }
        default:
            break;
    ...
    [self.backgroundImageView kk_setBackgroundColorFromColor:startColor toColor:endColor direction:UIViewGradientColorDirection_BottomTop];
}
...
- (void)playButtonClicked{
    if (self.stopPlayButton.tag==1110) {
        ...
        [self.stopPlayButton setImage:[self VideoPlay_Pause_ButtonImage]
                             forState:UIControlStateNormal];
        if (self.delegate && [self.delegate respondsToSelector:@selector(KKVideoPlayerBarView_PlayButtonClicked:)]) {
            [self.delegate KKVideoPlayerBarView_PlayButtonClicked:self];
        }
    }
    else{
        //暂停播放
        ...
        [self.stopPlayButton setImage:[self VideoPlay_Play_ButtonImage]
                             forState:UIControlStateNormal];
        if (self.delegate && [self.delegate respondsToSelector:@selector(KKVideoPlayerBarView_PauseButtonClicked:)]) {
            [self.delegate KKVideoPlayerBarView_PauseButtonClicked:self];
        }
    }
}

- (void)moreButtonClicked{
    if (self.delegate && [self.delegate respondsToSelector:@selector(KKVideoPlayerBarView_MoreButtonClicked:)]) {
        [self.delegate KKVideoPlayerBarView_MoreButtonClicked:self];
    }
}

- (void)backButtonClicked{
    if (self.delegate && [self.delegate respondsToSelector:@selector(KKVideoPlayerBarView_BackButtonClicked:)]) {
        [self.delegate KKVideoPlayerBarView_BackButtonClicked:self];
    }
}
...
- (void)sliderClicked:(UISlider*)slider{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(KKVideoPlayerBarView:currentTimeChanged:)]) {
        [self.delegate KKVideoPlayerBarView:self currentTimeChanged:_currentTime];
    }
}
...
- (UIImage*)VideoPlay_Play_ButtonImage{
    UIImage *image = [NSBundle kk_imageInBundle:@"KKVideoPlay_Resource.bundle" imageName:@"GUPlayer_Play2"];
    return image;
}

- (UIImage*)VideoPlay_Pause_ButtonImage{
    UIImage *image = [NSBundle kk_imageInBundle:@"KKVideoPlay_Resource.bundle" imageName:@"GUPlayer_Pause2"];
    return image;
}
...
- (UIImage*)VideoPlay_Cha_ButtonImage{
    UIImage *image = [NSBundle kk_imageInBundle:@"KKVideoPlay_Resource.bundle" imageName:@"GUPlayer_Cha"];
    return image;
}

- (UIImage*)VideoPlay_More_ButtonImage{
    UIImage *image = [NSBundle kk_imageInBundle:@"KKVideoPlay_Resource.bundle" imageName:@"GUPlayer_More"];
    return image;
}
